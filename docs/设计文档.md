...
┌─────────────────────────────────────────────────────────────────────┐
│                          SQLite 数据库                               │
│  papers表: pendingTitles → TobeDownloaded → processed → analyzed    │
└─────────────────────────────────────────────────────────────────────┘
```
...
当获取详情或PDF下载失败时，任务会被移入 `detail_failures` / `download_failures` 辅助表，
同时将 `papers.status` 设置为 `detailFailed` / `downloadFailed`，待人工执行
`scripts/retry_failures.py` 后重新回到主流程。
```
...
| status | TEXT | DEFAULT 'pendingTitles' | 处理状态 |
...
| paper_id | TEXT | UNIQUE | 关联论文ID |
| title | TEXT | NOT NULL | 失败的标题 |
| source | TEXT | - | 任务来源 |
| reason | TEXT | - | 失败原因 |
| failed_at | TEXT | DEFAULT CURRENT_TIMESTAMP | 失败时间 |
